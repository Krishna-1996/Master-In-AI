...
# A* search algorithm
def A_star_search(maze_obj, start=None, goal=None, heuristic_method=chebyshev_heuristic):
    if start is None:
...
if __name__ == '__main__':
    ...
    m.CreateMaze(loadMaze='...///My_Maze.csv')  # Adjust maze file path

    goal_position = (1, 1)  # Example goal position

    start_time = time.time()
    exploration_order, visited_cells, path_to_goal = A_star_search(m, goal=goal_position, heuristic_method=chebyshev_heuristic)
    end_time = time.time()
    ...
    update_info_window("Chebyshev", goal_position, path_length, search_length, execution_time, directional_weights)
