...
if __name__ == '__main__':
    ...
    # Common color for all search agents
    common_color = COLOR.orange

    # Create agents for visualization (same color for all pathfinding agents)
    a_star_agent = agent(m, footprints=True, shape='square', color=common_color)  # A* search agent
    greedy_bfs_agent = agent(m, footprints=True, shape='square', color=common_color)  # Greedy BFS search agent
    path_agent_a_star = agent(m, footprints=True, shape='star', color=common_color)  # A* path tracing agent
    path_agent_greedy_bfs = agent(m, footprints=True, shape='star', color=common_color)  # Greedy BFS path tracing agent
    path_goal_a_star = agent(m, 1, 1, footprints=True, shape='square', color=COLOR.blue, filled=True, goal=(m.rows, m.cols))  # Goal agent for A*
    path_goal_greedy_bfs = agent(m, 1, 1, footprints=True, shape='square', color=COLOR.red, filled=True, goal=(m.rows, m.cols))  # Goal agent for Greedy BFS
    ...
    textLabel(m, 'A* Path Length', len(path_to_goal_a_star))  # A* path length
    textLabel(m, 'Greedy BFS Path Length', len(path_to_goal_greedy_bfs))  # Greedy BFS path length
    textLabel(m, 'A* Search Length', len(exploration_order_a_star))  # A* search length
    textLabel(m, 'Greedy BFS Search Length', len(exploration_order_greedy_bfs))  # Greedy BFS search length
